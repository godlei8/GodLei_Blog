 ...
 *
 * <p>默认不启用。迁移时设置环境变量并启动一次：
 * <pre>
 *   BLOG_STORAGE_MIGRATE_LOCAL_TO_COS=true
 *   COS_SECRET_ID / COS_SECRET_KEY / COS_REGION / COS_BUCKET (必填)
 * </pre>
 * 迁移完成、核对无误后，再把该开关关掉并将上传 mode 切到 cos。
 */
@Slf4j
@Component
@RequiredArgsConstructor
@ConditionalOnProperty(name = "blog.storage.migrate-local-to-cos", havingValue = "true")
public class StorageMigrationRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        log.warn("[存储迁移] 请人工核对前台展示无误后：1) 关闭 blog.storage.migrate-local-to-cos；"
                + "2) 将 blog.storage.mode 切为 cos；3) 从仓库移除本地 uploads 目录。");
    }
}
