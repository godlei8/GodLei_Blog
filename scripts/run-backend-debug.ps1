...
$debugPort = if ($env:BACKEND_DEBUG_PORT) { $env:BACKEND_DEBUG_PORT } else { '5005' }
...
$jvmArgs = '-Dfile.encoding=UTF-8 -Duser.timezone=Asia/Shanghai -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:{0}' -f $debugPort
...
try {
  ...
  & $javaExe '-Dfile.encoding=UTF-8' '-Duser.timezone=Asia/Shanghai' "-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:$debugPort" '-jar' $backendJar '--spring.profiles.active=local'
} finally {
  Pop-Location
}
